...
2. Auth mixin layer — `OAuthClient` (OAuth 1.0a via the `oauth` gem; signs requests in `build_request` with `access_token.sign!`) and `OAuth2Client` (via `oauth2` >= 2.0.22 and < 3; injects `access_token.headers` in `build_request`, routes oauth2's Faraday traffic through a private `FilteredLogger`). Some clients bypass both: `FacebookClient` and `SlackClient` extend `Client` directly and handle their own auth (Facebook appends `access_token` + HMAC `appsecret_proof` to every request).
