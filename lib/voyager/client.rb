...
require 'logger'
require 'voyager/request'
require 'voyager/response'
...
module Voyager
  class Client
    ...
    def perform_request(method, path, body='', headers={})
      ...
      response = Voyager::Response.new(trace.id, http_response, response_parser)
    ...
    # add voyager standard headers...
    def add_standard_headers(headers={})
      headers.merge("User-Agent" => "voyager gem v#{Voyager::VERSION}")
    end
    ...
    def to_multipart_value(value)
      ...
      mime_type = Voyager::MIME.mime_type_for(value.path)
    ...
    def http_logger
      @http_logger ||= Voyager::Logging::HttpLogger.new
    end

    def logger
      @logger ||= (options[:logger] || Voyager.logger)
    end
  end
end
