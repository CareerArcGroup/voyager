...
module Voyager
  class Trace
    ...
    def self.filter(obj, filtered_terms)
      case obj
      when Voyager::Response
        obj
      when Voyager::Request
        Voyager::Request.new(obj.method, filter(obj.uri, filtered_terms), filter(obj.body, filtered_terms), filter(obj.headers, filtered_terms))
      when URI
    ...
    def include_log?
      Voyager.trace? || (response && !response.successful?)
    end
  end
end
